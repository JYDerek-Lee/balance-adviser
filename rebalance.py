...
def rebalance(balance):
    ...
    sellRatios = list(
        map(lambda a, r: (a - r) / a if a > r else 0, normAssets, normRatios)
    )
    buyRatios = list(
        map(lambda a, r: (r - a) / a if r > a else 0, normAssets, normRatios)
    )
    ...
    buyAmounts = dict(
        zip(
            keys, list(map(lambda a, r: math.floor(a * r), amounts(balance), buyRatios))
        )
    )
    change = 0
